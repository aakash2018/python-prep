...
8. Write a program that calculates the delta of a quadratic
 equation (Δ = b² - 4ac)
9. Write a program that calculates the delta of a quadratic
 equation (Δ = b² - 4ac)
10. Write a program that calculates the perimeter and area of a
...
99. Write a recursive function to determine whether a word is a
palindrome.
...
102. Make the BubbleSorth Algorithm
...
105.  Rawwords: Write an algorithm that checks whether a
word is a "rawword". A word is considered a "prime word" if the sum of the letter values (where 'a' = 1, 'b' = 2, etc.) is a prime number.
106. Implement an algorithm that takes an integer and
generates the Collatz sequence for that number. The Collatz sequence is generated by applying the following rules: if the number is even, divide it by 2; if the number is odd, multiply it by 3 and add 1. Repeat this process until you reach the number
